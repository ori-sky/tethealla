/************************************************************************
	...
	Copyright (C) 2020  David Farrell
...
typedef struct st_mag MAG;


/* Character Data Structure */

PACK(struct st_minichar {
...
typedef struct st_minichar MINICHAR;

//packetSize = 0x399C;
//command = 0x00E7;

PACK(struct st_bank_item {
...
typedef struct st_bank_item BANK_ITEM;
...
typedef struct st_item ITEM;

PACK(struct st_inventory {
	unsigned in_use; // 0x01 = item slot in use, 0xFF00 = unused
	unsigned flags; // 8 = equipped
	ITEM item;
});
typedef struct st_inventory INVENTORY;

PACK(struct st_chardata {
...
typedef struct st_chardata CHARDATA;
...
#define MAX_GM_MESG_LEN 4096
	...
	fprintf (fp, "[%u-%02u-%02u, %02u:%02u:%02u] %s", tm.tm_year+1900, tm.tm_mon, tm.tm_mday, 
		tm.tm_hour, tm.tm_min, tm.tm_sec, text);
	fclose (fp);

	printf ("[%u-%02u-%02u, %02u:%02u:%02u] %s", tm.tm_year+1900, tm.tm_mon, tm.tm_mday, 
		tm.tm_hour, tm.tm_min, tm.tm_sec, text);
}
			...
			if (config_data[0] != 0x23)
			{
				...
				case 0x05:
					// Server IP address
					{
						...
						{
							printf("Missing server IP address in config");
							printf("Hit [ENTER]");
							exit(1);
...
void SendB1 (BANANA* client)
{
	...
	if ((client->guildcard) && (client->slotnum != -1))
	{
		*(long long*) &client->encryptbuf[0] = *(long long*) &PacketB1[0];
		...
		sprintf (&client->encryptbuf[8], "%u:%02u:%02u: %02u:%02u:%02u.%03u", tm.tm_year+1900, tm.tm_mon, tm.tm_mday,
			...
			*(long long*) &client->hwinfo[0] = *(long long*) &client->decryptbuf[0x84];
			...
			case 0x00:
				...
				*(unsigned *) &client->encryptbuf[0x10] = gcn;
				...
				sprintf(client->guildcard_string, "%d", gcn); // auth'd
...
#ifdef NO_SQL
	LoadDataFile ("account.dat",   &num_accounts,   (void **)&account_data[0],   sizeof(L_ACCOUNT_DATA));
	LoadDataFile ("bank.dat",      &num_bankdata,   (void **)&bank_data[0],      sizeof(L_BANK_DATA));
	LoadDataFile ("character.dat", &num_characters, (void **)&character_data[0], sizeof(L_CHARACTER_DATA));
	LoadDataFile ("guild.dat",     &num_guilds,     (void **)&guild_data[0],     sizeof(L_GUILD_DATA));
	//LoadDataFile ("hwbans.dat",    &num_hwbans,     (void **)&hw_bans[0],        sizeof(L_HW_BANS));
	//LoadDataFile ("ipbans.dat",    &num_ipbans,     (void **)&ip_bans[0],        sizeof(L_IP_BANS));
	LoadDataFile ("keydata.dat",   &num_keydata,    (void **)&key_data[0],       sizeof(L_KEY_DATA));
	LoadDataFile ("security.dat",  &num_security,   (void **)&security_data[0],  sizeof(L_SECURITY_DATA));
	LoadDataFile ("shipkey.dat",   &num_shipkeys,   (void **)&ship_data[0],      sizeof(L_SHIP_DATA));
	LoadDataFile ("team.dat",      &num_teams,      (void **)&team_data[0],      sizeof(L_TEAM_DATA));
#endif
