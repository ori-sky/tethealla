...
#if defined(_MSC_VER)
#define TETH_PATH_MAX MAX_PATH
#elif defined(__unix__)
#include <limits.h>
#define TETH_PATH_MAX PATH_MAX
#else
#error Unable to define max path length
#endif
...
#define MAX_WELCOME_MESSAGE 4096
...
char Welcome_Message[MAX_WELCOME_MESSAGE] = {0};
...
typedef struct st_patch_data {
	unsigned file_size;
	unsigned checksum;
	char full_file_name[TETH_PATH_MAX+48];
	char file_name[48];
	char folder[TETH_PATH_MAX];
...
typedef struct st_banana {
	...
	int todc;
	char patch_folder[TETH_PATH_MAX];
...
char patch_folder[TETH_PATH_MAX] = {0};
...
void change_client_folder (unsigned patchNum, BANANA* client)
{
	...
	if (strcmp(&client->patch_folder[0], &s_data[patchNum].folder[0]) != 0)
	{
		...
		memcpy (&client->patch_folder[0], &s_data[patchNum].folder, TETH_PATH_MAX);
...
void change_patch_folder (unsigned patchNum)
{
	...
	if (strcmp(&patch_folder[0], &s_data[patchNum].folder[0]) != 0)
	{
		...
		memcpy (&patch_folder[0], &s_data[patchNum].folder, TETH_PATH_MAX);
